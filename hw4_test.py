...
dockerBuildTag = "shard" #put the tag for your docker build here
...
def getShardId(ipPort):
    print ("getShardId", str(ipPort))
    #return requests.get( 'http://192.168.0.1:8080/shard/my_id' )
...
class TestHW4(unittest.TestCase):
    ...
    def checkGetAllShardIds(self, ipPort, expectedStatus=200):
        ...
        print ("checkGetAllShardIds", response.json())
        self.assertEqual(response.status_code, expectedStatus)
        data = response.json()
    ...
    def checkGetMembers(self, ipPort, ID, expectedStatus=200, expectedResult="Success", expectedMsg=None):
        ...
        print ("checkGetMembers", response.json())
        self.assertEqual(response.status_code, expectedStatus)
        data = response.json()
    ...
    def getShardView(self, ipPort):
        ...
        print ("shardIDs", shardIDs)
        shardView = {}
        for ID in shardIDs:
            shardView[ID] = self.checkGetMembers(ipPort, ID)
        print ("shardView", shardView)
        return shardView

    def checkConsistentMembership(self, ipPort, ID):
        members = self.checkGetMembers(ipPort, ID)
        print ("checkConsistentMembership")
        for member in members:
            print ("maybe it's a type issue", members, ID)
